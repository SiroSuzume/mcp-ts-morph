...
### 未使用 export の検出 (`find_unused_exports_by_tsmorph`)

- **機能**: プロジェクト全体を走査し、宣言ファイルの外から参照されていない `export` を候補として列挙します。
...
- **ユースケース**: デッドコード掃除や、モジュールの公開面の棚卸し。削除前には必ず `find_references_by_tsmorph` でダブルチェックすること。
- **既知の限界**: 動的 `require` / `import()`、ファイルシステム規約に依存するルーティング（Next.js の `page.tsx` 等）、文字列リフレクション越しの参照は検出できません。これらは `entryPoints`（公開 API として常に使用扱い）や `excludeFilePatterns`（テスト・生成物などをスキャン対象外に）で候補を絞り込んでください。
- **`textOccurrences`**: 各候補ごとに、宣言ファイル以外のソース内で `\b<name>\b` が出現する回数を添える。`0` ならテキスト上もどこからも触れられていないため確度の高いデッド、`1+` なら文字列リテラル / JSX / 動的参照の可能性ありなので `find_references_by_tsmorph` で要確認。
- **オプション**: `entryPoints` (絶対パス配列)、`excludeFilePatterns` (部分一致)、`maxResults` (デフォルト 100)、`expandNamespaceImports` (デフォルト ON)。
