...
- **`textOccurrences`**: 各候補に、宣言ファイル以外のソース内で `\b<name>\b` が出現する回数を添えます。`0` ならテキスト上もどこからも触れられておらず確度の高いデッド、`1+` なら文字列リテラル / JSX / 動的参照の可能性があるため `find_references_by_tsmorph` で要確認。
- **オプション**: `entryPoints`（絶対パス配列。公開 API として常に使用扱い）、`excludeFilePatterns`（部分一致でスキャン対象外に）、`maxResults`（デフォルト 100）、`expandNamespaceImports`（デフォルト ON）。
